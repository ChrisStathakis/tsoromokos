...
from vendors.models import Vendor, TAXES_CHOICES, InvoiceItem
...
class Product(models.Model):
    ...
    price_buy = models.DecimalField(max_digits=20, decimal_places=5, verbose_name='Τελευταία Τιμή Αγοράς', default=0.00)
    value = models.DecimalField(max_digits=20, decimal_places=2, verbose_name='Τιμή Πώλησης', )

    average_price = models.DecimalField(max_digits=20, decimal_places=4, verbose_name='Μεση Τιμή', default=0.00)
    running_average_price = models.DecimalField(max_digits=20, decimal_places=4, verbose_name='Τρέχων Μεση Τιμή', default=0.00)
    ...
    def filters_data(request, qs):
        ...
        if PRODUCTION:
            ...
            if cate_name:
                qs = qs.filter(categories__in=cate_name)
            if q:
                second_qs = Product.objects.filter(id__in=InvoiceItem.my_query.return_products(q)) 
                qs = qs.filter(title__icontains=q.upper())
                qs = list(chain(qs, second_qs))
            if search_name:
                second_qs = Product.objects.filter(id__in=InvoiceItem.my_query.return_products(search_name)) 
