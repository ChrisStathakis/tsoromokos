...
class AnalysisIncomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_order = self.object_list.aggregate(Sum('order_cost'))[
            'order_cost__sum'] if self.object_list.exists() else 0
...
class AnalysisOutcomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        generic_expenses_analysis = generic_expenses.values('category__title').annotate(total=Sum('value')).order_by(
            '-total')
        ...
        months = sort_months([analysis_invoices_per_month, analysis_bills_per_month, payroll_analysis_per_month,
                              generic_expenses_analysis_per_month])
...
class CashRowView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        generic_expenses_total = generic_expenses.aggregate(Sum('value'))[
            'value__sum'] if generic_expenses.exists() else 0
...
class BalanceSheetView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        diff_total_6 = incomes_total_6 - expenses_total_6

        diff_total_13 = incomes_total_13 - expenses_total_13
        diff_total_24 = incomes_total_24 - expenses_total_24

        # general expenses fpa
        general_expenses_total_6 = general_expenses.aggregate(Sum('taxes_6'))['taxes_6__sum'] if general_expenses.exists() else 0
        general_expenses_total_13 = general_expenses.aggregate(Sum('taxes_13'))['taxes_13__sum'] if general_expenses.exists() else 0
        general_expenses_total_24 = general_expenses.aggregate(Sum('taxes_24'))['taxes_24__sum'] if general_expenses.exists() else 0
        general_expenses_total_taxes = general_expenses_total_6 + general_expenses_total_13 + general_expenses_total_24
        diff_total_taxes = incomes_total_taxes - expenses_total_taxes - general_expenses_total_taxes
        ...
        # this is a empiric way to calculate fpa
        taxes_mod = TaxesModifier.objects.first() if TaxesModifier.objects.exists() else None
        taxes_6, taxes_13, taxes_24 = [taxes_mod.six, taxes_mod.half, taxes_mod.normal] if taxes_mod else [15, 10, 75]
        incomes_total = float(incomes_total)
        payments_total = float(payments_total)
        tax_incomes = ((incomes_total * (float(taxes_6) / 100)) * 0.06) + (
                    (incomes_total * (float(taxes_13) / 100)) * 0.13) + (
                                  (incomes_total * (float(taxes_24) / 100)) * 0.24)
        tax_orders = ((payments_total * (float(taxes_6) / 100)) * 0.06) + (
                    (payments_total * (float(taxes_13) / 100)) * 0.13) + (
                                 (payments_total * (float(taxes_24) / 100)) * 0.24)
        tax_orders = tax_orders + (float(general_total) * 0.24)
