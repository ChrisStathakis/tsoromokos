...
class HomepageView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # vendors
        vendors_balance_qs = Vendor.objects.filter(balance__gte=0)
        vendors_cost = vendors_balance_qs.aggregate(Sum('balance'))['balance__sum'] if vendors_balance_qs.exists() else 0
        last_incomes = Income.objects.all()[:10]
        last_payments = VendorPayment.objects.all()[:30]

        # costumers
        invoices = Invoice.objects.all()[:30]
        context.update(locals())
        ten_days = datetime.now() + timedelta(days=20)
        context['paychecks'] = Paycheck.objects.filter(is_done=False, date__lte=ten_days)
        context['paycheck_money'] =  Paycheck.objects.filter(is_done=False, date__lte=ten_days).aggregate(Sum('value'))['value__sum'] if Paycheck.objects.filter(is_done=False, date__lte=ten_days).exists() else 0
        return context
...
class CostumerDetailView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        orders = Order.filters_data(self.request, self.object.orders.all())
        invoice_payments = self.object.payment_invoices.all()
...
def print_costumer_history_view(request, pk):
    instance = get_object_or_404(Costumer, id=pk)
    payments = Payment.filters_data(request, instance.payments.filter(is_paid=True))
    orders = Order.filters_data(request, instance.orders.all())
    total_payments = payments.aggregate(Sum('value'))['value__sum'] if payments.exists() else 0
    total_orders = orders.aggregate(Sum('value'))['value__sum'] if orders.exists() else 0
